...
class ChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if image_raw is not None:
            ...
            filename = ""
        form = await sync_to_async(Form.objects.get)(
            pk=self.scope["url_route"]["kwargs"]["id"]
        )
        form_message = await sync_to_async(FormMessage.objects.create)(
            message=message, image=filename, user=username
        )
        await sync_to_async(form.messages.add)(form_message)
        image_url = (
            form_message.image_thumbnail.url
            if form_message.image_thumbnail
            else ""
        )
