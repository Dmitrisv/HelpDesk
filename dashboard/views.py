...
def dashboard(request):
    ...
    workers_data = Form.objects.filter(done_at__date=date.today(), implementer__in=workers).values('implementer__last_name').annotate(count=Count('id'))

    workers_names = [worker['implementer__last_name'] for worker in workers_data]
